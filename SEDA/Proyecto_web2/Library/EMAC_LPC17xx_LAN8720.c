...
void init_ethernet (void) {
  ...
  //for (tout = 100; tout; tout--);        //DP83848C
  for (tout = 0; tout < 0x1000; tout++) ;  //LAN8720
  ...
  //LPC_EMAC->SUPP = SUPP_RES_RMII;             //DP83848C
  LPC_EMAC->SUPP = SUPP_RES_RMII | SUPP_SPEED;  // LAN8720
  //for (tout = 100; tout; tout--);             //DP83848C
  for (tout = 0; tout < 0x1000; tout++) ;       // LAN8720
  LPC_EMAC->SUPP = 0;
  ...
  /* Check if this is a LAN8720 PHY. */
    ...
    for (tout = 0; tout < 0x100000; tout++) {
	  ...
	  /* Check the link status. */
  for (tout = 0; tout < 0x10000; tout++) {
    //regv = read_PHY (PHY_REG_STS);//DP83848C
    regv = read_PHY (1); // LAN8720
    //if (regv & 0x0001) {  //DP83848C
//    if (regv & 0x0020) {    // LAN8720
    if (regv & 0x0004) {    // LAN8720
      /* Link is on. */
      break;
    }
  }

    regv = read_PHY (4); // LAN8720
	    regv = read_PHY (1); // LAN8720
    regv = read_PHY (5); // LAN8720
	    regv = read_PHY (1); // LAN8720
	    regv = read_PHY (31); // LAN8720
	    regv = read_PHY (1); // LAN8720
