...
/*Parámetros para el FUZZ*/
uint16_t fuzz_value=100; 
uint16_t input;
...
void ADC_IRQHandler(void)
{
  ...
  if (input > 250 + fuzz_value) input= 4095;		//Clippeamos/Recortamos al máximo
	if (input < 250 - fuzz_value) input= 0;			//""									"" al mínimo
